...
test("Cloud Functions build syncs the canonical Python package without cache files", async () => {
  await syncPython();
  const root = path.resolve(import.meta.dirname, "../../..");
  const source = path.join(root, "src", "mijia_agent");
  const output = path.join(root, "adapters/edgeone/cloud-functions/api/mijia_agent");
