...
Agent 负责面向模型的 JSON Schema，绝不将任意远程说明或 Schema 注入模型提示词。对于家庭相关问题，模型先选择本地发现工具；Agent 随后获取家庭授权清单，并生成受已批准房间、指标和设备类型约束的家庭读取工具 Schema。选中的读取工具只发起一次 `tools:invoke` 请求；控制台会再次校验当前可用性、授权和暴露范围。控制台复用一次设备发现结果，同时完成清单校验和所选采集器调用。读取环境数据时，它先将请求筛选条件与当前授权范围取交集，再采集对应指标并批量读取 MIoT 属性。Agent 发送请求前根据清单校验参数；控制台返回脱敏结果后，Agent 将结果与原始问题一起发送给模型生成最终答复。一般问题不会获取家庭授权清单。
...
实现进度（2026-09-24）：两个配套仓库已实现家庭级场景操作同意、规范化场景摘要、绑定版本的批准、保守的灯/开关风险筛选、Python 当前意图校验，以及控制台 Blob 认领/结果账本。远程写入仍受部署门控限制；剩余的 EdgeOne 并发、恢复和最终端到端检查见 [docs/TODO.md](./TODO.md)。在这些检查通过前，必须保持 AI_SCENE_EXECUTION_ENABLED 未设置。
