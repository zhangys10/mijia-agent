...
**选择性采用：** 精确、无歧义、低风险的命令可以走确定性快速路径，但必须使用与模型选择操作相同的策略和操作账本。LLM 仍负责一般问题和含糊表达。
...
| home_write_low | 经审核的场景激活 | 要求操作范围权限、明确的当前意图和持久化认领 |
...
| activate_scene | 低风险家庭写入 | 只接受本轮可信发现返回的别名 |

当操作范围、持久化账本、控制台执行门控或已批准场景版本不可用时，不暴露 activate_scene。
...
2. **操作前置条件：** 增加经过审核的场景别名、暴露及场景版本、风险分类、必要时的显式确认，以及由控制台负责的持久化 Blob 操作账本。必须在调用小米分发之前，基于主体、家庭、幂等键、规范操作哈希和版本原子创建账本认领。
...
| list_scenes | 返回已启用的手动场景，以主体/家庭范围的不透明别名、名称、通用说明和操作数量表示 | 复用别名机制；将粗略目录替换为带暴露、版本、风险和可搜索摘要的目录 |
...
| activate_scene | 校验 scope、别名、参数及幂等性，然后始终拒绝执行；预览只读 | 持续禁用，直到持久账本、版本绑定、风险策略和功能门控就绪 |
...
5. **场景摘要不足以安全选择。** Agent 只收到通用说明和操作数量，没有脱敏操作摘要、风险类别或版本。
...
Agent 负责面向模型的 JSON Schema，绝不将任意远程说明或 Schema 注入模型提示词。客户端需要发现能力时，可读取该清单。选中的读取工具只发起一次 tools:invoke 请求；控制台在该请求中检查当前可用性、授权和暴露范围。控制台复用一次设备发现结果，同时完成清单校验和所选采集器调用。读取环境数据时，它一次性采集所有已暴露指标，再根据请求中的房间/指标筛选条件在本地从脱敏快照中过滤。Agent 发送请求前先校验有界参数的形状；控制台则根据当前暴露投影校验每个筛选条件。
...
| find_scenes | 有长度上限的文本 query | 已暴露的不透明别名、版本、脱敏操作摘要和风险 |
...
- 扩展场景发现，加入暴露状态、规范化操作摘要、风险和版本哈希。
- 策略引擎。
- 由控制台负责的 EdgeOne Blob 操作账本，使用现有幂等键、不可变记录、onlyIfNew 和强一致性读取。
- 绑定版本的批准和显式执行门控。
- 最后执行一次低风险真实场景端到端验证。

实现进度（2026-09-25）：两个配套仓库已实现家庭级场景操作同意、规范化场景摘要、绑定版本的批准、保守的灯/开关风险筛选，以及控制台 Blob 认领/结果账本。实时场景目录会先解析设备房间和 MIoT 能力，再判定风险；批准版本会包含私有目标与动作信息，但不会将其暴露给模型。废弃的命令路由和自动化令牌工具入口目前均不可执行物理写入。canonical assistant 的动作 scope 注册和当前意图校验，须等 [docs/TODO.md](./TODO.md) 中的部署门禁通过后再完成。在此之前必须保持 AI_SCENE_EXECUTION_ENABLED 未设置。
