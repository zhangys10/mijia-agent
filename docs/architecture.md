...
build tree by the adapter's `build` script; the generated mirror is committed because
EdgeOne must package it from Git. Tests compare the source and mirror byte-for-byte.
